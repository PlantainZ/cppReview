...
int _tmain(int argc,_TCHAR* argv[]){
    ...
    scanf("%d",&m);
    for(i=1;i<N;i++) father[i]=-1; //110个全都填了-1

    for(i=0;i<m;i++){
        scanf("%d%d",&a,&b);//给出已连接的部分点。
        union_(a,b);
    }
    qsort(node,n*n,sizeof(Node),cmp);m
    //void qsort (void* base, size_t num, size_t size,
           //int (*compar)(const void*,const void*));
    //对node数组里，n*n个数，每个数大小为sizeof(Node)的东西，根据cmp结果进行排序
    //然后参照上边的cmp,小的排前边。
    MST_Kruskcal();
